...
class SchedulingPanel(JPanel implements Observer, ListSelectionListener, TableModelListener):

    queueTable = JTable()

    viewSaved = JButton()
    viewPublished = JButton()
    remove = JButton()
    ...
    private AbstractAction removeMatchButtonMethod():
        return new AbstractAction("Cancel Match"):
                ...
                if (queueTable.getSelectedRow() >= 0):
					...
					elif (state.equals("active")):
                        if (scheduler != null):
                            scheduler.abortOngoingMatch(matchId)
					elif (state.equals("aborted") || state.equals("done")):
    ...
    def void tableChanged(TableModelEvent arg0):
        updateButtonState()

    def void valueChanged(ListSelectionEvent arg0):
        updateButtonState()
    ...
    def void addPendingMatch(PendingMatch spec):
