 ...
 * that was selected.
 */
class SimpleDetailPanel(DetailPanel):
    moveTable = JZebraTable()

    def SimpleDetailPanel():
        ...
        self.add(new JScrollPane(moveTable, ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS, ScrollPaneConstants.HORIZONTAL_SCROLLBAR_AS_NEEDED), new GridBagConstraints(0, 0, 1, 1, 1.0, 1.0, GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(5, 5, 5, 5), 5, 5))

    def void observe(Event event):
        if (event instanceof GamerNewMatchEvent):
            observe((GamerNewMatchEvent) event)
		elif (event instanceof GamerSelectedMoveEvent):
