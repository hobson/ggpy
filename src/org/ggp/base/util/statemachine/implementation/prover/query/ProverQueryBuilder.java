...
class ProverQueryBuilder
{
    ...
    def static Set<GdlSentence> getContext(MachineState state)
	{
        return state.getContents();

    def static Set<GdlSentence> getContext(MachineState state, List<Role> roles, List<Move> moves)
	{
    ...
    def static GdlRelation getGoalQuery(Role role)
	{
        return GdlPool.getRelation(GOAL, new GdlTerm[] { role.getName(), VARIABLE });

    def static GdlRelation getInitQuery()
	{
        return INIT_QUERY;

    def static GdlRelation getLegalQuery(Role role)
	{
        return GdlPool.getRelation(LEGAL, new GdlTerm[] { role.getName(), VARIABLE });

    def static GdlRelation getNextQuery()
	{
        return NEXT_QUERY;

    def static GdlRelation getRoleQuery()
	{
        return ROLE_QUERY;

    def static GdlProposition getTerminalQuery()
	{
        return TERMINAL_QUERY;

    def static GdlRelation toDoes(Role role, Move move)
	{
