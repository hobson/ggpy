...
class AssignmentIteratorImpl(AssignmentIterator):
        ...
        private void updateNextAssignment():
            ...
            for(int i = 0; i < valueIndices.size(); i++):
				...
				elif(plan.getSourceDefiningSlot().get(i) == -1):
...
//					System.out.println("Setting based on a function: slot " + i + " to value " + valueFromFunction)
	    ...
	    def void changeOneInNext(Collection<GdlVariable> vars):
    	    ...
    	    def void changeOneInNext(Collection<GdlVariable> varsToChange,
    	    ...
    	    def Map<GdlVariable, GdlConstant> next():
            if(headOnly):
