...
class SentenceFormModelFactory(object):
	 ...
	 * on the description before using self. If the description allows
	 * for function-valued variables, some aspects of the model,
	 * including the dependency graph, may be incorrect.
	 */
    def static ImmutableSentenceFormModel create(List<Gdl> description) throws InterruptedException 
    ...
    def SetMultimap<SentenceForm, SentenceForm> augmentGraphWithLanguageRules(
        ...
        for (SentenceForm form : sentenceForms):
			...
			elif (form.getName() == GdlPool.DOES):
